...
bool validWifi(const String &name, const String &pass) {
  return name.length() >= 1 && name.length() <= 32 && pass.length() >= 8 &&
         pass.length() <= 63;
}
...
bool writeVault(const String &payload) {
  ...
  // Conserva una generaciÃƒÂ³n anterior para recuperarse de un corte durante el
  // guardado.
  LittleFS.remove("/vault.prev");
  if (LittleFS.exists("/vault.enc") &&
      !LittleFS.rename("/vault.enc", "/vault.prev")) {
    LittleFS.remove("/vault.tmp");
    return false;
  }
  if (!LittleFS.rename("/vault.tmp", "/vault.enc")) {
    if (LittleFS.exists("/vault.prev"))
      LittleFS.rename("/vault.prev", "/vault.enc");
    return false;
  }
  return true;
}
...
void setup() {
  Serial.begin(115200);
  if (!LittleFS.begin()) {
    Serial.println("LittleFS no disponible");
    return;
  }
  if (LittleFS.exists("/wifi.txt")) {
    ...
    wifiPass.trim();
    f.close();
    if (!validWifi(ssid, wifiPass)) {
      ssid = "SEC-VAULT-SETUP";
      wifiPass = "cambia-esto-ya";
    }
  }
  WiFi.mode(WIFI_AP);
  ...
  server.on("/api/save", HTTP_POST, [](AsyncWebServerRequest *r) {
    if (!r->hasParam("payload", true)) {
      r->send(400, "text/plain", "Falta payload");
      return;
    }
    String p = r->getParam("payload", true)->value();
    if (p.length() > MAX_PAYLOAD) {
      r->send(413, "text/plain", "Payload demasiado grande");
      return;
    }
    if (writeVault(p))
      r->send(200, "text/plain", "OK");
    else
      r->send(500, "text/plain", "No se pudo guardar");
  });

  server.on("/api/wifi", HTTP_POST, [](AsyncWebServerRequest *r) {
    if (!r->hasParam("ssid", true) || !r->hasParam("pass", true)) {
      r->send(400, "text/plain", "Faltan parámetros");
      return;
    }
    String n = r->getParam("ssid", true)->value(),
           p = r->getParam("pass", true)->value();
    if (!validWifi(n, p)) {
      r->send(400, "text/plain", "WiFi inválido");
      return;
    }
    ...
    if (!LittleFS.rename("/wifi.tmp", "/wifi.txt")) {
      r->send(500, "text/plain", "No se pudo aplicar");
      return;
    }
    r->send(200, "text/plain", "OK");
    rebootPending = true;
...
void loop() {
  if (rebootPending && millis() - rebootAt > 1500)
    ESP.restart();
}